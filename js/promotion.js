/**
 * Promotion.js - Updated with Referral Code Module (No Invites)
...
(function() {
    ...
    function init() {
        ...
        // Initialize Referral Code Module after delay
        setTimeout(function() {
...
window.InstallAppModule = (function() {
    ...
    function init() {
        ...
        if (core) userRef = core.getUserRef();
    ...
    async function checkIfAlreadyClaimed() {
        ...
        try {
            ...
            if (hasClaimedInstallReward) removeInstallBanner();
        } catch(e) { console.error('Check install reward error:', e); }
    }
    
    function checkIfRunningInApp() {
        const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true || window.matchMedia('(display-mode: fullscreen)').matches;
        ...
        if ((isStandalone || fromApp || appInstalledFlag === 'true') && !autoClaimAttempted) {
            ...
            if (!appInstalledFlag) localStorage.setItem('app_installed', 'true');
            if (!hasClaimedInstallReward) { console.log('🎉 App detected! Auto-claiming reward...'); autoClaimReward(); }
        }
    }
    
    async function autoClaimReward() {
        if (hasClaimedInstallReward) return;
        if (!userRef) { setTimeout(autoClaimReward, 1000); return; }
        try {
            const claimedCheck = await userRef.child('installRewardClaimed').once('value');
            if (claimedCheck.val() === true) { hasClaimedInstallReward = true; return; }
            if (window.PromotionCore) window.PromotionCore.addToBalance(INSTALL_REWARD, true);
            ...
            if (window.ConfettiModule) window.ConfettiModule.start();
            removeInstallBanner();
        } catch(e) { console.error('Error auto-claiming reward:', e); }
    }
    
    function showInstallSuccessNotification() {
        const notification = document.createElement('div');
        notification.className = 'install-success-notification';
        notification.innerHTML = `<div class="success-content"><img src="images/bonus150.png" alt="Bonus" style="width: 35px;"><div class="success-text"><strong>🎉 App Installed!</strong><span>You received ₱${INSTALL_REWARD} bonus!</span></div></div>`;
        notification.style.cssText = `position: fixed; top: 70px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, #1a1a2e, #0f0a1a); border: 2px solid #ffd700; border-radius: 16px; padding: 12px 20px; z-index: 10007; animation: slideDown 0.4s ease, fadeOut 0.4s ease 3s forwards; box-shadow: 0 5px 20px rgba(0,0,0,0.3); display: flex; align-items: center; gap: 12px; color: #ffffff;`;
        document.body.appendChild(notification);
        setTimeout(() => notification.remove(), 3500);
    }
    
    function setupInstallPrompt() {
        window.addEventListener('beforeinstallprompt', (e) => { e.preventDefault(); installPrompt = e; console.log('Install prompt ready'); const installBtn = document.getElementById('installBannerBtn'); if (installBtn) { installBtn.disabled = false; installBtn.innerHTML = '<span>INSTALL</span> <i class="fas fa-arrow-right"></i>'; } });
        window.addEventListener('appinstalled', () => { console.log('App was installed successfully'); localStorage.setItem('app_installed', 'true'); showPostInstallNotification(); });
    }
    
    function showPostInstallNotification() {
        const notification = document.createElement('div');
        notification.innerHTML = `<div style="display: flex; align-items: center; gap: 12px;"><span style="font-size: 24px;">✅</span><div><strong style="color:#ffd700;">App Installed!</strong><br><span style="font-size: 11px; color:#ffffff;">Open the app from home screen to claim ₱${INSTALL_REWARD} bonus!</span></div></div>`;
        notification.style.cssText = `position: fixed; top: 70px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, #1a1a2e, #0f0a1a); border: 2px solid #39ff14; border-radius: 16px; padding: 12px 20px; z-index: 10007; animation: slideDown 0.4s ease, fadeOut 0.4s ease 4s forwards; box-shadow: 0 5px 20px rgba(0,0,0,0.3);`;
        document.body.appendChild(notification);
    ...
    function createInstallBanner() {
        ...
        banner.innerHTML = `<div class="install-banner-card"><div class="install-icon-wrapper"><img src="images/bonus150.png" alt="Bonus" class="install-icon-img"><div class="install-pulse"></div></div><div class="install-banner-center"><div class="install-title">🎁 GET ₱${INSTALL_REWARD} BONUS!</div><div class="install-desc">Install app & claim automatically</div><div class="install-steps"><span>📱 Menu (⋮)</span><span>➜</span><span>🏠 Add to Home Screen</span><span>➜</span><span>✅ Install</span></div></div><div class="install-banner-right"><button class="install-action-btn" id="installBannerBtn"><span>INSTALL</span><i class="fas fa-arrow-right"></i></button><button class="install-close-btn" id="closeBannerBtn"><i class="fas fa-times"></i></button></div></div>`;
        ...
        if (installBtn) installBtn.onclick = function() { showInstallSteps(); };
    }
    ...
    function showInstallSteps() {
        ...
        if (isSafari) step1Text = 'Tap the <strong>Share icon (□↑)</strong> on your browser';
        else if (isChrome) step1Text = 'Tap the <strong>3 dots (⋮)</strong> menu icon';
        else if (isSamsung) step1Text = 'Tap the <strong>3 lines (☰)</strong> menu icon';
        const modal = document.createElement('div');
        modal.className = 'install-steps-modal';
        modal.innerHTML = `<div class="install-steps-card"><div class="steps-header"><div class="steps-icon">📲</div><div class="steps-title">Install Lucky Drop</div><button class="steps-close" id="closeStepsModalBtn">✕</button></div><div class="steps-body"><div class="step-item"><div class="step-number">1</div><div class="step-text">${step1Text}</div></div><div class="step-item"><div class="step-number">2</div><div class="step-text">Scroll down and tap <strong>🏠 Add to Home Screen</strong></div></div><div class="step-item"><div class="step-number">3</div><div class="step-text">Tap <strong>✅ Add</strong> to install the app</div></div><div class="step-item highlight"><div class="step-number">🎁</div><div class="step-text">After installation, <strong>open the app</strong> and <strong style="color:#00ff88;">₱${INSTALL_REWARD} will be auto-added</strong> to your balance!</div></div></div><div class="steps-footer"><button class="steps-btn" id="closeStepsBtn">Got it!</button></div></div>`;
    ...
    function removeInstallBanner() { const banner = document.getElementById('installAppBanner'); if (banner) banner.remove(); }
    return { init: init };
})();

// ========== REFERRAL CODE MODULE (NEW - NO INVITES) ==========
window.ReferralCodeModule = (function() {
    ...
    async function animateCodeGeneration(container, finalCode) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            slot.style.cssText = `display: inline-block; min-width: 50px; text-align: center; font-family: 'Orbitron', monospace; font-size: 32px; font-weight: 900; color: #fce883; text-shadow: 0 0 15px rgba(212, 175, 55, 0.5);`;
            slot.textContent = '?';
        ...
        for (let i = 0; i < 6; i++) {
            ...
            for (let r = 0; r < 15; r++) {
                ...
                setTimeout(() => { slot.style.transform = 'scale(1)'; slot.style.opacity = '1'; }, 50);
            }
            slot.textContent = finalChar;
            slot.style.animation = 'slotReveal 0.3s ease-out';
            if (window.PromotionCore) {
                try { const audio = new Audio('sounds/super_ace_scatter_ring.mp3'); audio.volume = 0.3; audio.play().catch(e => console.log('Sound error:', e)); } catch(e) {}
            }
    ...
    async function saveReferralCodeToDB(code) {
        ...
        try {
            ...
            return true;
        } catch(e) { console.error('Error saving referral code:', e); return false; }
    }
    ...
    async function loadReferralCode() {
        ...
        try {
            ...
            const code = snap.val();
            if (code) { currentReferralCode = code; console.log('✅ Loaded existing referral code:', code); return code; }
            return null;
        } catch(e) { console.error('Error loading referral code:', e); return null; }
    }
    
    function showToast(message) {
        const toast = document.createElement('div');
        toast.innerHTML = message;
        toast.style.cssText = `position: fixed; bottom: 100px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, #1a1a2e, #0f0a1a); border: 1px solid #d4af37; color: #fce883; padding: 10px 20px; border-radius: 50px; font-size: 12px; font-weight: bold; z-index: 10002; animation: fadeOutUp 2s ease-out forwards; white-space: nowrap; font-family: 'Orbitron', monospace;`;
        document.body.appendChild(toast);
        setTimeout(() => { if (toast) toast.remove(); }, 2000);
    }
    
    function renderReferralCodeUI(hasCode, code = null) {
        if (!referralCodeDisplay) { console.error('referralCodeDisplay element not found!'); return; }
        console.log('Rendering UI. hasCode:', hasCode, 'code:', code);
        if (!hasCode) {
            referralCodeDisplay.innerHTML = `<button class="golden-generate-btn" id="generateCodeBtn" style="cursor: pointer; width: 100%; padding: 18px; font-size: 18px; background: linear-gradient(135deg, #b8860b, #d4af37, #fce883, #d4af37); border: none; border-radius: 12px; font-family: 'Orbitron', monospace; font-weight: 900; color: #1a1100;"><i class="fas fa-gem"></i> 🪙 GENERATE REFERRAL CODE 🪙 <i class="fas fa-arrow-right"></i></button>`;
            const generateBtn = document.getElementById('generateCodeBtn');
            if (generateBtn) {
                const newBtn = generateBtn.cloneNode(true);
                generateBtn.parentNode.replaceChild(newBtn, generateBtn);
                newBtn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); handleGenerateCode(); });
                console.log('Generate button attached');
            }
        } else {
            referralCodeDisplay.innerHTML = `<div class="code-display-box" style="background: linear-gradient(135deg, #0a0a0a, #000); border: 2px solid #d4af37; border-radius: 16px; padding: 20px; text-align: center;"><div class="code-label" style="font-family: 'Orbitron', monospace; font-size: 10px; color: #d4af37;">⭐ YOUR PERMANENT REFERRAL CODE ⭐</div><div class="code-value" id="referralCodeValue" style="font-family: 'Orbitron', monospace; font-size: 32px; font-weight: 900; color: #fce883; letter-spacing: 6px;">${code}</div><div class="code-actions" style="margin-top: 15px;"><button class="copy-code-btn" id="copyCodeBtn" style="background: linear-gradient(to bottom, #0066ff, #0044cc); border: none; border-radius: 8px; padding: 8px 20px; font-family: 'Orbitron', monospace; color: white; cursor: pointer;"><i class="fas fa-copy"></i> COPY CODE</button></div></div>`;
            ...
            if (copyBtn) {
                ...
                newCopyBtn.addEventListener('click', function() {
                    ...
                    if (codeValue) {
                        navigator.clipboard.writeText(codeValue.textContent).then(() => { newCopyBtn.innerHTML = '<i class="fas fa-check"></i> COPIED!'; setTimeout(() => { newCopyBtn.innerHTML = '<i class="fas fa-copy"></i> COPY CODE'; }, 2000); showToast('✅ Referral code copied!'); }).catch(() => showToast('❌ Failed to copy'));
                    }
                });
            }
        }
    ...
    async function handleGenerateCode() {
        ...
        if (existingCode) { renderReferralCodeUI(true, existingCode); showToast('✅ You already have a referral code!'); return; }
        isGenerating = true;
        const generateBtn = document.getElementById('generateCodeBtn');
        if (generateBtn) { generateBtn.disabled = true; generateBtn.style.opacity = '0.6'; generateBtn.innerHTML = '<i class="fas fa-spinner fa-pulse"></i> GENERATING...'; }
        const newCode = generateReferralCode();
        console.log('Generated code:', newCode);
        const animationContainer = document.createElement('div');
        animationContainer.style.cssText = `display: flex; justify-content: center; gap: 8px; padding: 25px; background: linear-gradient(135deg, rgba(0,0,0,0.6), rgba(0,0,0,0.4)); border-radius: 16px;`;
        ...
        } else { renderReferralCodeUI(false); showToast('❌ Failed to save code. Please try again.'); }
        isGenerating = false;
    }
    ...
    async function init() {
        ...
        if (!currentUserPhone) { if (retryCount < MAX_RETRY) { retryCount++; setTimeout(init, 1000); } return; }
        const core = window.PromotionCore;
        if (core) userRef = core.getUserRef();
        referralCodeDisplay = document.getElementById('referralCodeDisplay');
        if (!referralCodeDisplay) { if (retryCount < MAX_RETRY) { retryCount++; setTimeout(init, 500); } return; }
        if (!userRef) { if (retryCount < MAX_RETRY) { retryCount++; setTimeout(init, 500); } return; }
        const existingCode = await loadReferralCode();
        if (existingCode) renderReferralCodeUI(true, existingCode);
        else renderReferralCodeUI(false);
        console.log('✅ Referral Code Module ready');
    }
    
    return { init: init, getReferralCode: () => currentReferralCode, generateNewCode: handleGenerateCode };
})();
